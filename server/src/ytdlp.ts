...
const getDlpPath = () => {
  ...
  // Verifica na pasta bin do diretório de trabalho atual (ex: se executado da raiz do workspace)
  const localExe = path.join(process.cwd(), 'bin', 'yt-dlp.exe');
  if (fs.existsSync(localExe)) return localExe;

  // Verifica na pasta pai (ex: se executado dentro da pasta server/)
  const parentExe = path.join(process.cwd(), '..', 'bin', 'yt-dlp.exe');
...
const getFfmpegPath = () => {
  ...
  // Tenta localizar no node_modules local
  const localFfmpeg = path.join(process.cwd(), 'node_modules', 'ffmpeg-static', isWin ? 'ffmpeg.exe' : 'ffmpeg');
  if (fs.existsSync(localFfmpeg)) {
    return localFfmpeg;
  }

  // Tenta localizar no node_modules da pasta pai
  const parentFfmpeg = path.join(process.cwd(), '..', 'node_modules', 'ffmpeg-static', isWin ? 'ffmpeg.exe' : 'ffmpeg');
